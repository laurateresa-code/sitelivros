...
import { Users, BookOpen, Plus, Loader2, MoreVertical, Edit, Share2, LogIn } from 'lucide-react';
...
export default function Clubs() {
  ...
  const fetchClubs = useCallback(async () => {
    ...
    try {
      ...
      setClubs(clubsWithRealCount as Club[]);
      ...
      if (user) {
        ...
        if (!membershipError && memberships) {
          setUserMemberships(new Set(memberships.map(m => m.club_id)));
        }
      }
    } catch (error) {
              ...
              return (
                <Card key={club.id} className="group hover:shadow-lg transition-shadow overflow-hidden">
                  ...
                  <CardContent className="space-y-4">
                    ...
                    {isMember ? (
                      <Button className="w-full" onClick={() => handleEnterClub(club.id)}>
