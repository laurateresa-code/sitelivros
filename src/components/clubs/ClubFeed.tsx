...
export function ClubFeed({ clubId }: ClubFeedProps) {
  const { posts, loading, error, likePost, unlikePost, refresh, deletePost } = useFeed(clubId);
    ...
    <div className="space-y-6">
      ...
      {error && (
        <Alert variant="destructive">
          ...
          <AlertDescription>
            {error.message?.includes('club_id') || error.message?.includes('schema cache') 
              ? 'O banco de dados precisa ser atualizado para suportar o feed do clube. Contate o administrador.' 
